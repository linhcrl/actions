...
versions=$(grep -E "^## \[[0-9]+\.[0-9]+\.[0-9]+\]" "$changelog" || true)

if [ -z "$versions" ]; then
  echo "No versions found, skipping order validation"
  exit 0
fi
...
if [ "$version_error" = true ] || [ "$date_error" = true ]; then
  echo ""
  echo "CHANGELOG versions must be in descending order (newest first)"
  exit 1
fi

echo "Version and date ordering is correct"
